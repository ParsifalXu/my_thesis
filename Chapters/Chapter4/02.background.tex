\section{Background}\label{sec:bg-mpchecker}
This section reviews the essential terminology and background necessary for understanding the subsequent chapters.
\subsection{Multi-Parameter Constraints}\label{sec:eg-mpchecker}
Two examples illustrate inconsistencies between API documentation and the corresponding code
...
\fref{fig:eg1} illustrates an inconsistency caused by an explicit constraint from the class
\textit{AutoReg}.
...
\fref{fig:eg2} displays an inconsistency caused by an implicit constraint from the class
...
However, the above constraint does not faithfully reflect the behavior implemented in code.
...
This reveals that the documentation does not describe all branches in which \texttt{gamma} is ignored. The stated condition itself can still be sufficient, so the distinction between incompleteness and a violated implication matters when checking this example (\sref{sec:fuzzword-mpchecker}).

For this type of implicit constraint, traditional pattern-based approaches are not able to extract
the \emph{doc-constraint} correctly and thus fail to detect the inconsistencies.
To solve this issue, a customized constraint incorporating fuzzy words is designed, and few-shot learning is adopted to teach LLMs how to generate such constraints (details in
\sref{sec:llm-mpchecker}).
...
\textit{x} is ignored (see \sref{sec:fuzzword-mpchecker}).
...
\end{tcolorbox}
        \label{fig:eg2-code}
    \end{subfigure}
    \caption{Examples of an implicit constraint from \texttt{Scikit-learn}.}
    \label{fig:eg2}
    \vspace{-5pt}
\end{figure*}
...
complete falsity, and 1 represents absolute truth.
