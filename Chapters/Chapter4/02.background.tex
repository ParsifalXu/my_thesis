...
This reveals that the documentation does not describe all branches in which \texttt{gamma} is ignored. The stated condition itself can still be sufficient, so the distinction between incompleteness and a violated implication matters when checking this example (\sref{sec:worked-mpchecker}).
