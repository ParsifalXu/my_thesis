...
In addition, when it comes to constraint extraction, compared to single-parameter constraints, we need to classify the multi-parameter constraint extraction problem into two types, as discussed in \sref{sec:eg-mpchecker}, 1) explicit constraint and 2) implicit constraint.

\toolchecker aims to accurately extract multi-parameter constraints from API documentation and detect both types of inconsistency. As the architecture depicted in \fref{fig:arch}, we have designed a three-phase workflow comprising the \textbf{1) Data Preprocessing; 2) Constraint Extraction; 3) Inconsistency Detection}. During the preprocessing phase, we separate the code and documentation within the project.
