...
\begin{figure*}[h]
    ...
    \caption{The architectural overview of \tool.}
    \label{fig:arch}
    \vspace{-5pt}
\end{figure*}
...
In addition, when it comes to constraint extraction, compared to single-parameter constraints, we need to classify the multi-parameter constraint extraction problem into two types, as discussed in Section~\ref{sec:eg}, 1) explicit constraint and 2) implicit constraint.

\tool aims to accurately extract multi-parameter constraints from API documentation and detect both types of inconsistency. As the architecture depicted in Figure~\ref{fig:arch}, we have designed a three-phase workflow comprising the \textbf{1) Data Preprocessing; 2) Constraint Extraction; 3) Inconsistency Detection}. During the preprocessing phase, we separate the code and documentation within the project.
\tool will then automatically rewrite each function to be compatible with the symbolic
