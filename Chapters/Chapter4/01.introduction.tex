\section{Introduction}\label{sec:introduction-mpchecker}
...
Well-designed API documentation not only provides detailed descriptions of interfaces,
...
using the APIs.
However, frequent version updates may cause the documentation to become out of sync with the corresponding
...
On the other hand, the number of parameters/attributes in the APIs of these libraries can be significantly greater
than that in a typical library API, sometimes over a few dozen.
...
(3) For the same reason, a semantics-aware code analysis approach is essential, as logical relations among
...
To detect multi-parameter constraint inconsistencies from data science library documentation, this chapter proposes an automated tool, \toolchecker.
\toolchecker identifies inconsistencies between API documentation and the corresponding library code by
combining symbolic execution-based program analysis techniques with constraint extraction methods
powered by large language models (LLMs).
\toolchecker first extracts multi-parameter constraints from documentation (a.k.a. \emph{doc-constraints}),
leveraging the powerful natural language understanding capability of LLMs.
The extraction process incorporates a few optimizations, such as Chain of Thought (CoT)~\cite{wei2022chain} and few-shot
learning to improve the accuracy of constraint extraction.
Then \toolchecker uses dynamic symbolic execution to collect all path constraints from the corresponding
Python source code.
...
LLMs, a \emph{Fuzzy Constraint Logic} (FCL) framework is designed and implemented to estimate how
...
Through fuzzy constraint satisfaction, \toolchecker can accommodate many \emph{nearly-correct} constraints
produced by LLMs and thus improve the accuracy of the overall approach.


\paragraph{Contributions.}
This chapter aims to integrate precise symbolic reasoning with the inherently fuzzy outputs of large language models.
Specifically, this chapter makes the following contributions.
\begin{enumerate}
    \item An automated multi-parameter code-documentation inconsistency detection technique was proposed, and an end-to-end command-line tool called \toolchecker was developed. Existing techniques in the same area are only designed to handle single parameter inconsistencies, without considering inter-parameter constraints.
    \item A customized fuzzy constraint satisfaction framework was introduced to mitigate the
    uncertainties introduced by LLM outputs. A theoretical derivation of the membership function based on constraint similarity is provided.
    \item The study constructed a documentation constraint dataset comprising 72 real-world constraints sourced from widely used data science libraries, and derived a mutation-based inconsistency dataset with 216 constraints.
    The dataset and tool implementation are made available online: \url{https://github.com/ParsifalXu/MPChecker}.
    \item \toolchecker was evaluated on four real-world popular data science libraries. Fourteen inconsistency issues discovered by \toolchecker were reported to the developers, who have confirmed 11 inconsistencies at the time of writing.
\end{enumerate}