...
As shown in the red box in \fref{fig:arch-dataloc}, before executing an LLM-generated Datalog program, the framework enforces a \emph{parser-gated validation} workflow to ensure that only syntactically well-formed
...
An intermediate-rule diagnostic mechanism is introduced for LLM-generated Datalog programs. As
...
\noindent\textbf{Mutation catalog.}
...
\item \textsc{M2. Match-Literal}: replace each body string literal
with~$V_i$ and append \texttt{match("s.*", $V_i$)}, testing whether the
literal is a prefix of the actual fact value.
\item \textsc{M3. Drop-Single-Atom}: remove one selected body atom at
...
too strong.
\end{itemize}
M1--M2 apply only when quoted string literals occur in the rule body; M3--M5
apply only when their target atom exists and the remaining body still contains
at least one atom.

\noindent\textbf{Feedback construction.}
