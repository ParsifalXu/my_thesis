...
Program facts are extracted through a modular pipeline that separates source discovery, structural parsing, and fact emission, shown as the offline \textit{Program Fact Extraction} stage in \fref{fig:arch-dataloc}. First, the extraction pipeline enumerates source files under the repository root using configurable include/exclude patterns that respect version-control ignores, build artifacts, and generated code. A language-specific frontend then analyzes the files using the most suitable intermediate representation. For Python, the frontend parses source code into an abstract syntax tree and emits facts during traversal. The frontend produces Datalog facts annotated with precise source locations and stable identifiers, enabling traceability and incremental updates.
