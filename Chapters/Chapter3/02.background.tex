...
By contrast, regression testing~\cite{yoo2012regression} operates within the specific context of a given client application and can be used to validate modifications to client-side code. Nevertheless, regression testing frequently fails to identify incompatibilities that arise from library upgrades. This shortcoming stems from the fact that regression testing suites are constructed by client developers, who often lack comprehensive knowledge about library modifications.


% 增加关于AI的讨论，如何把AI结合进来等等。application scenarios中增加AI的部分。


\section{Dataset Creation}\label{creation}

This section outlines the methodology and process employed to
...
To ensure the representativeness and reproducibility of the \toolcompsuite dataset, the selection process focused on high-quality and popular client projects and
...
popular projects, all the available projects were systematically sorted
in descending order based on their number of stars on GitHub and the target clients were selected from the top of the list. The selection of
libraries was sourced from Maven Central~\cite{mvnrepo}, which hosts
33.5M Java libraries and their associated binaries, making it
a widely used repository of libraries for Java API and library
research~\cite{mostafa2017study,wu2016exploratory,qiu2016understanding,kula2014visualizing}. A library was included in the dataset only if it had more than 100 usages (i.e., clients) on Maven
Central. The selection criteria aimed to ensure the inclusion of
...
Among the highly-rated client projects, the selection criteria focused
...
libraries in the dataset is simplified.

\tref{client-library} presents the top 5 client projects and libraries in the \toolcompsuite dataset, ranked by popularity. For
each client project, the table provides its lines of code (LoC) and number of GitHub stars; for each library, it lists its number of usages by other projects from Maven Central.
...
These statistics suggest that the incompatibility issues present in
...
the runs are reproducible, each project underwent a series of checks. First, the project was checked out to the version (SHA) at the time of
the dataset creation, referred to as the \emph{base version}.
Next, the standard Maven project compilation command was executed to verify whether the project could be compiled successfully. Projects that failed to compile were excluded from the dataset. Subsequently, the standard Maven test command was executed to run all the tests in the project, ensuring that all tests passed on the base version. Any project that failed to pass tests at this stage was excluded. Finally, only client projects that successfully compiled and passed all tests on the base version were included,
thereby ensuring that the dataset consists only of projects which can
...
\begin{figure*}
  \centering
  \includegraphics[width=\linewidth]{Figures/Chapter3/architecture.pdf}
...
The upper left portion of \fref{fig:architecture} illustrates the approach taken by \toolcompsuite to identify incompatibilities
between a client project and its dependent libraries. Specifically,
for each client project on its base version, each dependent library was upgraded individually, and tests were run to determine whether the upgrade caused any failures. The intuition behind this approach is that since all the
...
introduced incompatibility issues. A test that flips from passing to failing is termed an \emph{incompatibility-revealing test}.

To automatically upgrade the libraries and run the tests, the data collection process used the Maven Versions Plugin~\cite{mvnplugin}. For a given client project, its dependency list was scanned using this plugin to identify all the
libraries that had newer versions available on Maven Central. If a
library had a newer version, it was marked as upgradable. Next, for
each upgradable library, the plugin was used to upgrade it by updating
the \Code{pom.xml} file to the most recent version on Maven
Central. The test suite of the client was then re-executed. If any
tests failed during this run, the client-library pair was marked as having an incompatibility issue, and the test was marked as an incompatibility-revealing test of this issue. It is crucial to note
that only one library was upgraded at a time to isolate failures caused
by different libraries. To ensure the accuracy and dependability of
the dataset, each identified incompatibility issue was manually verified. In particular, the test failure messages and reports were carefully examined to confirm that they were indeed
caused by the upgraded library. For each incompatible client-library
pair, a single incompatibility-revealing test was selected for inclusion in the final dataset. In cases where a client-library pair
had multiple incompatibility issues, the issue judged most representative and easy to comprehend was selected.
...
Finally, the metadata of all the selected incompatibility issues were persisted in a collection of \Code{json} files.
