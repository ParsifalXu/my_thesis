...
This section provides instructions on the usage of the dataset.
...
facilitate the demonstration of such issues, the dataset includes annotated checkpoints in the version histories of the client projects and tags that guide users to explore any incompatibility issues
present in the \toolcompsuite dataset.

As illustrated on the right-hand side of \fref{fig:architecture}, the
...
information. To mark the base version of the project, the \Code{git tag}~\cite{gittag} command was used to designate the base version as $V_{base}$. Subsequently, a patch was developed to upgrade the library from its old version to its new version,
...
applied to the $V_{base}$ version, resulting in a new version identified as $V_{incomp}$. Notably, the only difference between
...
\fref{fig:architecture}, the client project employs version 2.2.1 of
...
exhibit incompatibility issues in the client project, a separate branch named after each library is created, along with a $V_{incomp}$ version tag for each, with accompanying annotations that
...
An automated tool, named \runner, was also developed as part of \toolcompsuite (shown at the bottom of \fref{fig:architecture}). With \runner, users can easily reproduce and investigate any
incompatibility issue in a one-click manner by providing the issue ID
as input.

\runner offers an option which enables users to reproduce an
...
When \runner runs, it clones the client project from the forked
...
\runner also provides a set of commands that break down the entire cycle of incompatibility
...
Several other \runner commands are available for users to inspect
